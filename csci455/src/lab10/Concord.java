...
public class Concord {
    ...
    List<Map.Entry<String, Integer>>result = new ArrayList<Map.Entry<String, Integer>>();
    	...
    	Collections.sort(result,new Concord.ConcordComparator());
    	for(int i = 0; i<result.size(); i++) {
    		out.println(result.get(i).getKey()+" "+result.get(i).getValue());
    	}
    }
     ...
     */
    public void printSatisfying(PrintStream out, LargeWordPred pred) {
    ...
    public static class ConcordComparator implements Comparator{
    ...
    public static class LargeWordPred implements Predicate{
